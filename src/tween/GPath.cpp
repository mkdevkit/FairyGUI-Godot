...
NS_FGUI_BEGIN
static std::vector<Vector3> splinePoints;

GPathPoint::GPathPoint(const Vector3& pos)
{
    this->pos = pos;
    this->control1 = Vector3();
    this->control2 = Vector3();
    this->curveType = CurveType::CRSpline;
}

GPathPoint::GPathPoint(const Vector3& pos, const Vector3& control)
{
    this->pos = pos;
...
GPathPoint::GPathPoint(const Vector3& pos, const Vector3& control1, const Vector3& control2)
{
    this->pos = pos;
...
GPathPoint::GPathPoint(const Vector3& pos, CurveType curveType)
{
    this->pos = pos;
    this->control1 = Vector3();
    this->control2 = Vector3();
...
void GPath::create(GPathPoint* points, int count)
{
    ...
    for (int i = 1; i < count; i++)
    {
        ...
        if (prev->curveType != GPathPoint::CurveType::CRSpline)
        {
            ...
            seg.length = prev->pos.distance_to(current->pos);
...
void GPath::createSplineSegment()
{
    ...
    seg.length = 0;
    for (int i = 1; i < cnt; i++)
        seg.length += splinePoints[i - 1].distance_to(splinePoints[i]);
    _fullLength += seg.length;
    _segments.push_back(seg);
    splinePoints.clear();
}
...
Vector3 GPath::getPointAt(float t)
{
...
void GPath::getPointsInSegment(int segmentIndex, float t0, float t1,
                               std::vector<Vector3>& points, std::vector<float>* ts, float pointDensity)
{
    if (ts != nullptr)
        ts->push_back(t0);
    ...
    else if (seg.type == GPathPoint::CurveType::Bezier || seg.type == GPathPoint::CurveType::CubicBezier)
    {
        ...
        int SmoothAmount = (int)std::min<float>(seg.length * pointDensity, 50.0f);
...
void GPath::getAllPoints(std::vector<Vector3>& points, float pointDensity)
{
    int cnt = (int)_segments.size();
    for (int i = 0; i < cnt; i++)
...
Vector3 GPath::onCRSplineCurve(int ptStart, int ptCount, float t)
{
...
Vector3 GPath::onBezierCurve(int ptStart, int ptCount, float t)
{
    float t2 = 1.0f - t;
