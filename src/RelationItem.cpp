...
void RelationItem::addRefTarget(GObject* target)
{
    ...
    if (target != _owner->_parent)
        target->addEventListener(UIEventType::PositionChange, [this](EventContext* ctx) { RelationItem::onTargetXYChanged(ctx); }, EventTag(this));
    target->addEventListener(UIEventType::SizeChange, [this](EventContext* ctx) { RelationItem::onTargetSizeChanged(ctx); }, EventTag(this));
