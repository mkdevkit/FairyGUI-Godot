...
GTree::~GTree()
{
    // // CC_SAFE_RELEASE removed - _rootNode managed by Godot ref counting;
}
...
void GTree::createCell(GTreeNode* node)
{
    ...
    if (cc != nullptr)
    {
        cc->addEventListener(UIEventType::Changed, [this](EventContext* ctx) { GTree::onExpandedStateChanged(ctx); });
    ...
    if (node->isFolder())
        child->addEventListener(UIEventType::TouchBegin, [this](EventContext* ctx) { GTree::onCellTouchBegin(ctx); });
...
void GTree::_bind_methods()
{
    ...
    ClassDB::bind_method(D_METHOD("collapseAll", "folder_node"), &GTree::collapseAll);
