...
Vector2 TValue::getVec2() const
{
    return Vector2(f1, f2);
}

void TValue::setVec2(const Vector2& value)
{
    f1 = value.x;
    f2 = value.y;
}

Vector4 TValue::getVec4() const
{
    return Vector4(f1, f2, f3, f4);
}

void TValue::setVec4(const Vector4& value)
{
    f1 = value.x;
    f2 = value.y;
    f3 = value.z;
...
Color TValue::getColor() const
{
    return Color(f1, f2, f3, f4);
}

void TValue::setColor(const Color& value)
{
    f1 = value.r;
    f2 = value.g;
    f3 = value.b;
...
{
    FGUI_DELETE(path);
    FGUI_DELETE(startValue);
    FGUI_DELETE(endValue);
}
...
void Transition::play(int times, float delay, float startTime, float endTime, PlayCompleteCallback onComplete, bool reverse)
{
    ...
    if (delay == 0)
        ...
        GTween::delayedCall(delay)->setTarget(this)->onComplete([this]() { Transition::onDelayedPlay(); });
}
...
void Transition::setPaused(bool paused)
{
    ...
    for (auto& item : _items)
    {
        ...
        else if (item->type == TransitionActionType::Animation)
        {
            if (paused)
            {
                ((TValue_Animation*)item->value)->flag = item->target->getProp(ObjectPropID::Playing).operator bool();
                item->target->setProp(ObjectPropID::Playing, Variant(false));
            }
            else
                item->target->setProp(ObjectPropID::Playing, Variant(((TValue_Animation*)item->value)->flag));
        }
...
void Transition::setValue(const std::string& label, const Array& values)
{
    for (auto& item : _items)
    {
        void* value;
        ...
        case TransitionActionType::Skew:
        {
            ...
            tvalue->b1 = true;
            tvalue->b2 = true;
            tvalue->f1 = values[0].operator double();
            tvalue->f2 = values[1].operator double();
            break;
        }

        case TransitionActionType::Alpha:
        case TransitionActionType::Rotation:
            ((TValue*)value)->f1 = values[0].operator double();
            ;
            break;

        case TransitionActionType::Color:
        {
            uint32_t v = values[0].operator int();
            ((TValue*)value)->setColor(Color(((v >> 16) & 0xFF) / 255.0f, ((v >> 8) & 0xFF) / 255.0f, (v & 0xFF) / 255.0f, ((v >> 24) & 0xFF) / 255.0f));
            break;
        }
        ...
        case TransitionActionType::Animation:
        {
            ...
            tvalue->frame = values[0].operator int64_t();
            if (values.size() > 1)
                tvalue->playing = values[1].operator bool();
            break;
        }

        case TransitionActionType::Visible:
            ((TValue_Visible*)value)->visible = values[0].operator bool();
            break;

        case TransitionActionType::Sound:
        {
            TValue_Sound* tvalue = (TValue_Sound*)value;
            tvalue->sound = std::string((const char*)values[0].stringify().utf8().ptr());
            if (values.size() > 1)
                tvalue->volume = values[1].operator double();
            break;
        }
        ...
        case TransitionActionType::Transition:
        {
            ...
            tvalue->transName = std::string((const char*)values[0].stringify().utf8().ptr());
            if (values.size() > 1)
                tvalue->playTimes = values[1].operator int64_t();
            break;
        }

        case TransitionActionType::Shake:
        {
            ((TValue_Shake*)value)->amplitude = values[0].operator double();
            if (values.size() > 1)
                ((TValue_Shake*)value)->duration = values[1].operator double();
            break;
        }
        ...
        case TransitionActionType::ColorFilter:
        {
            ...
            tvalue->f1 = values[0].operator double();
            tvalue->f2 = values[1].operator double();
            tvalue->f3 = values[2].operator double();
            tvalue->f4 = values[3].operator double();
            break;
        }

        case TransitionActionType::Text:
        case TransitionActionType::Icon:
            ((TValue_Text*)value)->text = std::string((const char*)values[0].stringify().utf8().ptr());
            break;
        default:
            break;
        }
...
void Transition::setTimeScale(float value)
{
    if (_timeScale != value)
    {
        ...
        for (auto& item : _items)
        {
            ...
            {
                if (item->target != nullptr)
                    item->target->setProp(ObjectPropID::TimeScale, Variant(value));
            }
        }
    }
}
...
void Transition::playItem(TransitionItem* item)
{
    ...
    if (item->tweenConfig != nullptr)
    {
        ...
        if (_endTime == -1 || time <= _endTime)
        {
            ...
            item->tweener->setDelay(time)
                ...
                ->setTargetAny(item)
                ->onStart([this](GTweener* tweener) { Transition::onTweenStart(tweener); })
                ->onUpdate([this](GTweener* tweener) { Transition::onTweenUpdate(tweener); })
                ->onComplete1([this](GTweener* tweener) { Transition::onTweenComplete(tweener); });
    ...
    else if (item->type == TransitionActionType::Shake)
    {
        ...
        if (_endTime == -1 || time <= _endTime)
        {
            value->lastOffset = Vector2();
            value->offset = Vector2();
            item->tweener = GTween::shake(Vector2(), value->amplitude, value->duration)
                ->setDelay(time)
                ->setTimeScale(_timeScale)
                ->setTargetAny(item)
                ->onStart([this](GTweener* tweener) { Transition::onTweenStart(tweener); })
    ...
    else
    {
        ...
        else if (_endTime == -1 || time <= _endTime)
        {
            ...
            item->tweener = GTween::delayedCall(time)
                ->setTimeScale(_timeScale)
                ->setTargetAny(item)
                ->onComplete1([this](GTweener* tweener) { Transition::onDelayedPlayItem(tweener); });
        }
    }
...
void Transition::skipAnimations()
{
    ...
    for (int i = 0; i < cnt; i++)
    {
        ...
        target = item->target;
        frame = target->getProp(ObjectPropID::Frame).operator int64_t();
        playStartTime = target->getProp(ObjectPropID::Playing).operator bool() ? 0 : -1;
        playTotalTime = 0;
        ...
        target->setProp(ObjectPropID::Playing, Variant(playStartTime >= 0));
        target->setProp(ObjectPropID::Frame, Variant(frame));
        if (playTotalTime > 0)
            target->setProp(ObjectPropID::DeltaTime, Variant(playTotalTime));
    }
}
...
void Transition::applyValue(TransitionItem* item)
{
    ...
    case TransitionActionType::Color:
        item->target->setProp(ObjectPropID::Color, Variant(ToolSet::colorToInt((Color)((TValue*)item->value)->getColor())));
        break;
    ...
    case TransitionActionType::Animation:
    {
        ...
        if (value->frame >= 0)
            item->target->setProp(ObjectPropID::Frame, Variant(value->frame));
        item->target->setProp(ObjectPropID::Playing, Variant(value->playing));
        item->target->setProp(ObjectPropID::TimeScale, Variant(_timeScale));
        break;
    }
