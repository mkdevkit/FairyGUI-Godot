...
void GearXY::apply()
{
    ...
    if (_tweenConfig && _tweenConfig->tween && UIPackage::_constructing == 0 && !disableAllTweenEffect)
    {
        ...
        Vector2 originPt(_owner->getX(), _owner->getY());
        ...
        if (originPt != endPt)
        {
            ...
            _tweenConfig->_tweener = GTween::to(originPt, endPt, _tweenConfig->duration)
                                         ...
                                         ->setTargetAny(this)
                                         ->onUpdate([this](GTweener* tweener) { GearXY::onTweenUpdate(tweener); })
                                         ->onComplete([this]() { GearXY::onTweenComplete(); });
        }
    }
    else
    {
...
{
    _storage[_controller->getSelectedPageId()] = Vector4(
        _owner->getX(),
        _owner->getY(),
        _owner->getX() / _owner->getParent()->getWidth(),
        _owner->getY() / _owner->getParent()->getHeight());
...
void GearXY::updateFromRelations(float dx, float dy)
{
    if (_controller != nullptr && !_storage.empty() && !positionsInPercent)
    {
        ...
        {
            it->second = Vector4(it->second.x + dx, it->second.y + dy, it->second.z, it->second.w);
        }
        _default.x += dx;
        _default.y += dy;
