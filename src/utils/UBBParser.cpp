...
void UBBParser::onTag_IMG(const std::string & tagName, bool end, const std::string & attr, std::string& replacement)
{
    if (!end)
    {
        ...
        if (defaultImgWidth != 0)
            replacement = "<img src=\"" + src + "\" width=\"" + std::string((const char*)Variant(defaultImgWidth).stringify().utf8().ptr()) + "\" height=\"" + std::string((const char*)Variant(defaultImgHeight).stringify().utf8().ptr()) + "\"/>";
        else
